...
public class Mipmaps
{
    ...
    public static Dimension[] makeMipmapDimensions(final int width, final int height, final String iconName) {
        ...
        if (texWidth != width || texHeight != height) {
            Config.warn(invokedynamic(makeConcatWithConstants:(Ljava/lang/String;II)Ljava/lang/String;, iconName, width, height));
            return new Dimension[0];
        }
        final List listDims = new ArrayList();
        ...
        while (true) {
            ...
            final int mipLen = mipWidth * mipHeight * 4;
            final Dimension dim = new Dimension(mipWidth, mipHeight);
            listDims.add(dim);
        }
        final Dimension[] mipmapDimensions = listDims.toArray(new Dimension[listDims.size()]);
    ...
    public static IntBuffer[] makeMipmapBuffers(final Dimension[] mipmapDimensions, final int[][] mipmapDatas) {
        ...
        for (int i = 0; i < mipmapDimensions.length; ++i) {
            ...
            final IntBuffer buf = Config.createDirectIntBuffer(bufLen);
